...
- Der Browser-Testhost startet die veröffentlichte Server-EXE aus einem frisch erzeugten `dotnet publish`-Verzeichnis und verwendet dieses als Content Root. Die dedizierte Browser-Testdatenbank wird beim Start über die normalen Migrationen einschließlich Initial-Seed in den benötigten Zustand gebracht; sie ist nie die von `ManualDatabaseIntegration` destruktiv verwendete Datenbank. Readiness, Circuitzustand und Interaktionen werden ausschließlich über beobachtbare Zustände und Playwright-Web-first-Assertions abgewartet; feste Sleeps sind verboten. Der Host wird auch bei Testfehlern beendet und der gebundene Port freigegeben.
