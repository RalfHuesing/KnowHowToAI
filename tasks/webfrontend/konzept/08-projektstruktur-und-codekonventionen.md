...
- JavaScript ist nur für fehlende Browser-/Komponentenfunktionen zulässig und wird per JS-Isolation featurelokal gehalten. Der native Dialogwrapper bleibt eine dünne Interopgrenze; `ContentEditor.razor.js` besitzt für den Crepe-Adapter begrenzten Instanz-/Dispose-Zustand und bleibt auf die vier Aufrufe `mount`, `readMarkdown`, `focus`, `dispose` beschränkt.
...
| Produktives JavaScript mit Adapterzustand | `vitest` | ausschließlich `src/KnowHowToAI.Server/Frontend`; `npm test` über `scripts/test-fast.ps1`, ohne Browserstart |
