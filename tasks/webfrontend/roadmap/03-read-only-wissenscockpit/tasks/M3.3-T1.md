...
- [x] **M3.3-T1 – Lazy-Loading-Datenadapter für den Wissensbaum implementieren**
  - Umfang: den einzelnen Root über `NavigationService.GetRootAsync` laden; pro Expand `NavigationService.ListChildrenAsync` mit explizitem `ReadContext`, `AudienceId`, `Limit: 100` und dem unverändert weitergereichten opaken Cursor aufrufen. `ChildCount > 0` bestimmt `HasChildren`; kein separater HTTP-Endpunkt und kein neuer Persistence-Port.
  ...
  - Tests: bUnit-Adapter-/Pagingtests instrumentieren jeden `GetRootAsync`-/`ListChildrenAsync`-Aufruf und prüfen Parent, Zielgruppe, Read Context, `Limit = 100`, Cursor, Aufrufanzahl, Cache-Eviction und dass nie ein Vollbaum angefordert oder im ViewModel gehalten wird.
