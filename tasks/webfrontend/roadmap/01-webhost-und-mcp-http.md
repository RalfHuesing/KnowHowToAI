...
- [x] **M1.3 abschließen**
  ...
  - [x] **M1.3-T1 – MCP-HTTP-Transport produktiv integrieren**
    ...
    - Integrationsfixture: realen Kestrel-Host auf dynamischem Loopback-Port starten und ausschließlich `McpClient` plus `HttpClientTransport` `2.2.0` des offiziellen SDK mit `TransportMode = StreamableHttp` verwenden. Keine direkten Endpointaufrufe und kein selbstgebautes JSON-RPC im Vertragstest.
