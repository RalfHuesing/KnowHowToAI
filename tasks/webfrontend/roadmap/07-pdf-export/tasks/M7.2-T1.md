# M7.2-T1 – PDF-Application-Service aus dem bestehenden Teilbaumexport bauen

[Milestone-Roadmap](../roadmap.md)

## M7.2 – Konvertierung

- [ ] **M7.2-T1 – PDF-Application-Service aus dem bestehenden Teilbaumexport bauen**
  - Umfang: Node, Zielgruppe und Read Context an `export_tree` übergeben; Markdown mit Template per Pandoc und `--pdf-engine=weasyprint` konvertieren.
  - Semantik: Root exportiert alles, innerer Node nur sich und Nachfahren; TODOs bleiben unverändert enthalten.
  - Tests: Root, Teilbaum, Zielgruppen-Fallback, leerer Inhalt und ungültiger Kontext.
  - Abnahme: Service liefert gültige PDF-Bytes ohne Webabhängigkeit.
