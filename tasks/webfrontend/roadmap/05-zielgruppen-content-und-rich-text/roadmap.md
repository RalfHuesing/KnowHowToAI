# M5 – Zielgruppen-Content und Rich Text
...
Zielgruppen, Resolution Orders und zielgruppenabhängiger Markdown-Content können in einer Working Transaction ohne Agent gepflegt werden. WYSIWYG- und Markdown-Quellmodus verwenden dasselbe kanonische Markdown, Fallback/Provenienz/Freshness bleiben sichtbar, und jede Mutation ist gegen stale Writes geschützt.
...
- Alle Zielgruppen- und Content-Writes übergeben `TransactionId` und `expectedChangeVersion`; stale Writes werden atomar und strukturiert abgelehnt. Die neue `ChangeVersion` wird bis Web-UI und MCP transportiert.
...
- Fallback ist read-only. „Eigenen Inhalt anlegen“ und „Fallback als Ausgangstext übernehmen“ sind explizite Aktionen; kein stilles Bearbeiten oder Duplizieren der Quellzielgruppe. Explizit leerer Content unterdrückt Fallback und erfordert Bestätigung; Löschen ist separat und reaktiviert Fallback.
- Moduswechsel ist explizit. `Derived` benötigt mindestens eine über Node-Suche und Zielgruppe ausgewählte aktive explizite Source und pinnt ihre aktuelle Revision; freie GUID-Eingabe ist ausgeschlossen. Die Resolution-Order-Vorschau verwendet einen auswählbaren Beispiel-Node und sortiert gespeicherte Reihenfolgen nicht still um.
...
- [x] **M5.0 abschließen**
  ...
  - Entschieden: Zielgruppenadministration bleibt Bestandteil von M5/V1; Markdown-Quellmodus wird aufgenommen; npm + Lockfile + esbuild ist die lokale Buildtoolchain; Fallback-, Leerinhalt-, Independent-/Derived- und Resolution-Order-Semantik ist festgelegt.
...
- [x] **M5.1 abschließen**
  - [x] [M5.1-T1 – Zielgruppen- und Content-Writes gegen ChangeVersion absichern](tasks/M5.1-T1.md)
...
- [x] **M5.2 abschließen**
  ...
  - [x] [M5.2-T3 – Markdown-Quellmodus integrieren](tasks/M5.2-T3.md)

## M5.3 – Zielgruppenverwaltung

- [ ] **M5.3 abschließen**
  - [x] [M5.3-T1 – Zielgruppen in der Working Transaction pflegen](tasks/M5.3-T1.md)
  - [ ] [M5.3-T2 – Resolution Orders und Fallback-Vorschau pflegen](tasks/M5.3-T2.md)

## M5.4 – Zielgruppen-Content
...
- [ ] **M5.5 abschließen**
  - [ ] [M5.5-T1 – Zielgruppen-/Content-Workflows mit Navigation, Reconnect und Parallelität abnehmen](tasks/M5.5-T1.md)
...
- [ ] Zielgruppen und Resolution Orders sind ohne MCP in einer Working Transaction pflegbar.
