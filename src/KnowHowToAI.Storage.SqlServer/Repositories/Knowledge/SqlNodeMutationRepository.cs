...
internal sealed class SqlNodeMutationRepository : SqlRepository, INodeMutationRepository
{
        ...
        if (!SetEquals(previousState.Dependencies, currentState.Dependencies))
        {
            ...
            if (currentState.Dependencies.Count > 0)
            {
                await context.ExecuteAsync(
                    InsertDependencySql,
                    currentState.Dependencies.Select(ToDependencyParameters),
                    cancellationToken).ConfigureAwait(false);
            }
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var inserts = currentContents.Where(content => !previousByKey.ContainsKey((content.NodeId, content.RoleId)))
            .Select(ToContentParameters).ToArray();
    ...
    private static object ToContentParameters(NodeContent content) => new
    {
    ...
    private static object ToDependencyParameters(ContentDependency dependency) => new
    {
