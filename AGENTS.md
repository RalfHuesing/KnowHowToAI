...
Hierarchische, versionierte und zielgruppenabhängige Wissensbasis für Agenten und Menschen (.NET / C#, MS SQL Server, MCP Streamable HTTP).
...
- Maximale Token-Effizienz ist verbindlich: keine unnötigen Agenten, keine doppelten Analysen oder Prüfungen, keine ungekürzten Logwiedergaben und keine vorsorgliche Kontextflut. Entscheidungsrelevante Informationen, Nachweise und Sicherheitsgrenzen dürfen dabei nicht verloren gehen. Umfangreiche Rechercheergebnisse oder Rohdaten lagern Subagenten in isolierte Markdown-Dateien unter `temp/agents/<task-oder-thema>/` aus und übergeben dem Hauptagenten nur Kernaussage und Pfad.
