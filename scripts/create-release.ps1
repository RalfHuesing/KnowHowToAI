...
.DESCRIPTION
    1. Liest die Version aus src/KnowHowToAI.Cli/KnowHowToAI.Cli.csproj
...
$ProjectFile = 'src/KnowHowToAI.Cli/KnowHowToAI.Cli.csproj'
$VersionPattern = '(?<=<Version>)\d+\.\d+\.\d+(?=</Version>)'

function Get-RepoRoot {
    $root = git rev-parse --show-toplevel 2>$null
    if (-not $root) {
        throw 'Kein Git-Repository gefunden. Bitte im KnowHowToAI-Repo ausfuehren.'
    }
...
function Wait-ReleaseWorkflow {
    ...
    if (-not (Test-GhAuthenticated)) {
        ...
        Write-Host '       https://github.com/RalfHuesing/KnowHowToAI/actions/workflows/release.yml'
        return
    }
...
try {
    ...
    Write-Host ''
    Write-Host "KnowHowToAI Release" -ForegroundColor White
