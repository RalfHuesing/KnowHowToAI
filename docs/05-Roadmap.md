...
* **Inhaltliche Validierung von `content`** (nicht nur Struktur). `DocsValidator` prüft aktuell nur YAML/Slug/Hierarchie (siehe [04, Abschnitt 3](04-Datenmodell-Validierung-Edgecases.md#3-validierungsregeln-validate)), nicht den Markdown-`content`-Text selbst. **Konkreter, bereits aufgetretener Fehlerfall:** Ein Agent hat in echten Docs Links wie `[Buchungserfassung](file:///c:/Daten/.../erfassung-sitzungen.md)` geschrieben — Datei-/`file://`-Referenzen sind im SQL-Cache-Modell aber falsch bzw. für das lesende LLM wirkungslos, da dort kein Dateisystem mehr existiert; Navigation läuft ausschließlich über `list_children`/`get_doc`/Slugs (siehe [02, Abschnitt 4.D](02-Architektur-und-Techstack.md#d-knowhowtoaicli-server---config-path)). Sollte `validate` künftig als Fehler melden, analog zu den bestehenden Regeln (Datei + Fundstelle in der Fehlerliste).
  * **Technisch machbar ohne LLM-Call**, per Regex/Markdown-Link-Parsing auf den `content`-Text: Linkziele mit `file://`-Schema, sowie Linkziele die auf `.md`/`.markdown` enden (relativ oder absolut) — beides deutet auf eine Datei-basierte statt Slug-basierte Referenz hin.
  * **Denkbar, aber mehr Aufwand:** interne Querverweise zwischen Dokumenten gegen die tatsächlich vorhandenen Slugs prüfen (tote interne Links erkennen) — setzt aber erst eine Design-Entscheidung voraus, wie ein "Verweis auf ein anderes Dokument" syntaktisch überhaupt aussehen soll (aktuell nicht definiert, siehe [01](01-Konzept-und-Workflow.md)).
  * **Bewusst nicht vorgeschlagen:** Live-Erreichbarkeits-Check von `http(s)://`-Links — `validate` ist laut [04, Edge Case 4.8](04-Datenmodell-Validierung-Edgecases.md#48-verbindung-zum-sql-server-nicht-erreichbar) bewusst rein dateibasiert ohne Netzwerkabhängigkeit; ein Netzwerk-Check würde `validate` langsam und flaky machen.
  * **Warnung bei zu langen Einzeldokumenten** (eigener Auslöser: mehrere Zehntausend Zeichen in einer einzigen `.md`-Datei sind vorgekommen und mussten von Hand identifiziert und umstrukturiert werden). `content` wird laut [04, Edge Case 4.6](04-Datenmodell-Validierung-Edgecases.md#46-sehr-große-dokumente) nie trunkiert — `get_doc` liefert immer den vollen Text ans LLM, ein sehr großes Einzeldokument bläht also direkt dessen Kontext auf und spricht für eine feinere Slug-Aufteilung (Kern-Idee der hierarchischen Bibliothek, siehe [00-Overview.md](00-Overview.md)). Vorschlag: Zeichenlänge von `content` prüfen, **Default-Schwelle 8.000 Zeichen** (grob 2.000 Token — genug für ein ausführliches Einzelthema, aber ein klares Signal, dass eine Datei eigentlich mehrere Slugs sein sollte). **Nur Warnung, kein Fehler** — `validate` listet die Datei mit Zeichenzahl, bricht aber nicht ab (Exit-Code bleibt 0, wenn sonst alles valide ist). Schwelle gehört nach [06-configuration.mdc](../.agents/rules/06-configuration.mdc) als Einstellung nach `appsettings.json` (z.B. `KnowHowToAi:Validation:MaxContentLengthWarning`), nicht hartcodiert — je nach Wissensdatenbank kann ein sinnvoller Wert abweichen.
