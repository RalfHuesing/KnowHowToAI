...
* **`SqlDocumentsStore` ohne Unit-Tests** (bewusst — dünner DB-Adapter, siehe [03, Abschnitt 1](03-Projektstruktur-und-Konfiguration.md#1-solution-layout)). Wenn die `LIKE`-Brace-Escape-Logik (`BuildLikePattern`) oder die Insert-Reihenfolge nach Slug-Tiefe getestet werden sollen, ohne echten SQL-Server zu brauchen: **SQLite-In-Memory als Test-Backend** evaluieren. Vorbehalt: `LIKE`-Semantik und `NVARCHAR(MAX)`-JSON-Operationen verhalten sich zwischen SQLite und SQL Server leicht unterschiedlich, false-positive/false-negative-Tests sind möglich. Aufwand grob: ~2-4 Stunden Infrastruktur-Setup + ~1-2 Stunden pro Methode. Nicht für v1.
* **Bulk-Import mit `SqlBulkCopy`** (Performance-Optimierung, ab >5.000 Dokumenten sinnvoll): `SqlDocumentsStore.ReplaceAllAsync` macht aktuell N+1-Inserts in einer Transaktion (ein Round-Trip pro Dokument). Bei <1.000 Dokumenten auf lokalem SQL-Server ca. 0,5-1 Sekunde, kein Problem. Bei großen Bibliotheken (z.B. 50.000 Rewe-Produkt-Artikel): `SqlBulkCopy` mit `DestinationTableName` + manuellem Column-Mapping. Aufwand: ~2h + 30 Min Tests. Nicht für v1, beim ersten realen Bedarf angehen.
