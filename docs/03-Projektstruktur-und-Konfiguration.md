...
* **Credential-Strategie für v1 (bewusst gewählt):** die committete `appsettings.json` enthält Demo-Credentials (`Agent`/`Agent!`) — kein echtes Geheimnis, nur ein lokal akzeptierter Login. Beim ersten produktiven Einsatz umstellen auf eines der Standard-Pattern: (a) `appsettings.example.json` als Template committen + `appsettings.json` in `.gitignore` aufnehmen, oder (b) `dotnet user-secrets` für lokale Dev-Secrets, oder (c) Umgebungsvariablen-Override (`KnowHowToAi__ConnectionString`) im CI/Produktiv-System. Siehe [.agents/rules/06-configuration.mdc](../.agents/rules/06-configuration.mdc) für die Konfigurations-Disziplin.
...
* **`Microsoft.Data.SqlClient 7.0.2` ist gepinnt.** Pinning auf 7.0.2 (nicht 7.0.0) ist korrekt, weil 7.0.1 den `SqlBulkCopy`-Fix für SQL Server 2016 bringt (graph_type-Column-Metadata-Bug). Für dieses Projekt irrelevant (kein `SqlBulkCopy`-Use-Case), aber das Pinning schützt vor versehentlichem Re-Pinning auf 7.0.0. Bei SQL-Server-Versionen ≤ 2016 mit `SqlBulkCopy`-Bedarf ist stattdessen die 6.x-Linie zu verwenden — kein automatisches Downgrade in v1. Weitere 7.0-Breaking-Changes (Entra-ID muss explizit referenziert werden, `SqlVector` jetzt `readonly struct`, `ActiveDirectoryPassword` als Obsolete) sind für den aktuellen SQL-Login-Use-Case irrelevant.
* **`TrustServerCertificate=True`:** im lokalen Dev-Setup zwingend, weil die lokale SQL-Server-Instanz typischerweise selbst-signierte Zertifikate verwendet und sonst die Verbindung mit Zertifikatsfehler abbricht. Für produktive Setups mit echten CA-Zertifikaten sollte der Wert auf `False` stehen oder die Direktive komplett entfernt werden — dann prüft der Treiber die Zertifikatskette normal.
* **NuGet-Audit-Mode:** in beiden `*.csproj` aktiv (`<NuGetAuditMode>direct</NuGetAuditMode>`, `<NuGetAuditLevel>high</NuGetAuditLevel>`). `dotnet restore` läuft den Audit automatisch und blockiert bei `high`/`critical` Vulnerabilities. `scripts/publish.ps1` ruft zusätzlich `dotnet list package --vulnerable` als Smoke-Check vor dem Publish auf — fail-fast bei hochriskanten Paketen, moderate/low werden durchgelassen und manuell entschieden.

### MCP-Launch-Konfiguration (Beispiel für MCP-Hosts)
...
* **Regel-Sync:** Der Test ruft `--sync-agent-rules --agent-rules-path .agents/rules` mit auf (ab AiNetLinter 1.0.77) und hält `.agents/rules/AiNetLinter.mdc` (generierte Grenzwert-Übersicht) automatisch aktuell — `.agents/rules/` ist die eine Quelle der Wahrheit für Agenten-Regeln (siehe [00-Overview.md](00-Overview.md), Grundsatzentscheidung 1 in [04-docs-reference.mdc](../.agents/rules/04-docs-reference.mdc)). Diese Datei ist automatisch generiert — nicht manuell bearbeiten, Anpassungen gehören in `KnowHowToAI.rules.json`.
...
* **Self-contained statt framework-dependent:** Die .NET-Runtime ist eingebettet (~85 MB statt ~1-5 MB). Bewusst gewählt, damit die `.exe` unabhängig davon läuft, ob ein MCP-Host den MCP-Server-Prozess mit einer .NET-Runtime im `PATH` startet — derselbe Grund wie bei der `%COMPUTERNAME%`-Auflösung (Abschnitt 2): der MCP-Host-Prozess-Kontext ist nicht garantiert identisch mit einer normalen interaktiven Shell.
