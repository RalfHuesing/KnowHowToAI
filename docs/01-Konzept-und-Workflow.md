...
| --- | --- | --- |
| **Der Entwickler** | Steuert das System, gibt fachliche Impulse, stößt den Sync-Prozess an. | CLI, AI-Agent, Git |
| **Der Co-Writer (AI-Agent)** | Schreibt neue Dokumente, strukturiert bestehende Doku um, pflegt Metadaten. | Editor, Markdown-Dateien |
...
| **Der MCP-Server** (`KnowHowToAI.Cli server`) | Übersetzt die strukturierten SQL-Daten in mundgerechte Häppchen für das LLM während der Arbeit. | MCP-Client / AI-Agent |
...
│  Entwickler & Agent schreiben/refactoren Doku.         │
└───────────────────────────┬────────────────────────────┘
                            ▼
┌────────────────────────────────────────────────────────┐
│                  3. VALIDIEREN (Gate)                   │
│  CLI-Validator prüft YAML-Front-Matter, Slugs & Pfade.  │
│  Bei Fehlern: Agent repariert die Dateien.             │
└───────────────────────────┬────────────────────────────┘
                            ▼
┌────────────────────────────────────────────────────────┐
│                   4. IMPORTIEREN (Sync)                 │
...
Du arbeitest mit einem AI-Agenten. Du fragst das LLM: *"Wie war nochmal das Routing für den Core-Switch im IT-Bereich eingerichtet?"*
...
Du beauftragst den Agenten: *"Erstelle mir eine neue Doku für unsere DNS-Konfiguration im IT-Bereich."*

1. Der Agent exportiert bei Bedarf den aktuellen Stand ins lokale Zielverzeichnis (`KnowHowToAI.Cli export`).
2. Der Agent analysiert die Ordnerstruktur und sieht `it/netzwerk/routing.md`.
3. Der Agent entscheidet autonom, eine neue Datei unter `it/netzwerk/dns.md` anzulegen — mit einem **strikt regelkonformen Slug** (nur `a-z`, `0-9`, `-`).
4. Der Agent schreibt den Text und befüllt das YAML Front Matter (Title, Tags, Synonyme) sauber im Header der Datei. Title/Tags/Synonyme dürfen normales Deutsch inkl. Umlauten enthalten — nur der Dateipfad (Slug) unterliegt der strikten Regel.

**Leeres/fremdes docs-root:** Sitzt ein Agent in einem anderen Projekt an einem leeren docs-root, fehlen ihm die Beispiel-Dateien aus Schritt 2 als Vorlage. Dafür liefert der MCP-Server das Format selbst: eine kurze Instruktion bei jeder Verbindung plus die Resource `docs://authoring-guide` mit Front-Matter-Template, Slug-Regeln und Hierarchie-Regel (siehe [02, Abschnitt 4.D](02-Architektur-und-Techstack.md#d-knowhowtoaicli-server---config-path)).
...
* **Szenario A (Fehler):** Der Agent hat im YAML-Header ein Komma vergessen, einen ungültigen Slug (`Änderung.md`) verwendet oder den Parent-Ordner falsch benannt (Orphan). Der Validator bricht ab und listet alle Fehler mit Datei + Grund. Du sagst dem Agenten: *"Behebe die Validierungsfehler in `it/netzwerk/dns.md`."*
