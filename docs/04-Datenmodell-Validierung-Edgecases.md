...
**Keine Transaktion über die Skript-Liste.** Der Runner führt jedes Skript über `connection.ExecuteAsync(...)` aus, ohne explizit eine Transaktion um die gesamte Schleife zu legen — jeder `ExecuteAsync` auto-committet. Beim aktuellen Skript-Set (nur `0001_create_documents_table.sql`, idempotent) ist das irrelevant, weil das Skript beim zweiten Lauf gar nichts tut. Sobald ein zweites, *nicht*-idempotentes Skript hinzukommt (z.B. ein Index-Add), wird das ein Risiko: Skript 1 committed, Skript 2 fehlschlägt → DB in halb-migriertem Zustand. Beim Anlegen eines zweiten Skripts daher entweder (a) den Runner so erweitern, dass er alle Skripte in einer `BeginTransaction`/`Commit` umschließt, oder (b) explizit dokumentieren, dass jedes Skript unabhängig idempotent und vor SQL-Fehlern sicher sein muss.
...
**Performance-Erwartung `LIKE '%...%'`:** `LIKE` mit führendem Wildcard verhindert SQL-Server-Index-Nutzung — der Server macht einen vollen Tabellen-Scan + Pattern-Match pro Zeile. Grobe Orientierung auf einem typischen lokalen SQL-Server: <1.000 Dokumente < 100 ms (kein Problem), ~10.000 Dokumente ~0,5-1 s (merklich), >100.000 Dokumente mehrere Sekunden (interaktive LLM-UX unbrauchbar). Mitigation bis zur Migration auf SQL-Server-Full-Text: `KnowHowToAi.Search.MaxResults` (Default 50, siehe [03, Abschnitt 2](03-Projektstruktur-und-Konfiguration.md#2-konfiguration-appsettingsjson)) kappt die Antwortmenge auf das LLM-Token-Budget, das SQL-Query selbst läuft aber weiter über alle Zeilen. Bei wachsenden Bibliotheken FTS-Migration evaluieren.
