...
[Trait("Category", "Unit")]
public sealed class DraftLifecycleTests : BunitContext
{
    ...
    public void Commit_AfterConfirmation_CommitsWithMessageAndSwitchesToCurrentSnapshot()
    {
        ...
        Assert.Equal(KnowHowToAI.Server.Web.State.KnowledgeReadContextKind.Current, _workspaceState.CurrentContext.ReadContext);
    ...
    public void Commit_WithSnapshotConflict_ShowsBaseCurrentComparisonAndStartsManualReapply()
    {
        ...
        Assert.EndsWith("/drafts/00000000-0000-0000-0000-000000000402", BrowserNavigation.Uri, StringComparison.Ordinal);
        Assert.Empty(_toastState.Entries);
    }
    ...
    [Fact]
    public void CompletedTransaction_ShowsReadOnlyCompletionState()
    {
        _repository.Transaction = CommittedTransaction();

        var cut = RenderPage();

        Assert.Single(cut.FindAll("[data-testid='draft-completed']"));
        Assert.Equal("/knowledge", cut.Find("[data-testid='draft-open-current']").GetAttribute("href"));
        Assert.Empty(cut.FindAll("[data-testid='transaction-completion']"));
    }

    private IRenderedComponent<DraftPage> RenderPage() =>
        Render<DraftPage>(parameters => parameters.Add(page => page.TransactionId, _repository.Transaction.TransactionId.Value));

    private NavigationManager BrowserNavigation => Services.GetRequiredService<NavigationManager>();

    private static IRenderedComponent<TransactionCompletionPanel> CompletionPanel(IRenderedComponent<DraftPage> cut) =>
        cut.FindComponent<TransactionCompletionPanel>();

    private static void ConfirmCommit(IRenderedComponent<DraftPage> cut) =>
        CommitDialog(cut).Find(".confirmation-dialog__button--primary").Click();

    private static IRenderedComponent<ConfirmationDialog> CommitDialog(IRenderedComponent<DraftPage> cut) =>
        cut.FindComponents<ConfirmationDialog>()[0];

    private static IRenderedComponent<ConfirmationDialog> DiscardDialog(IRenderedComponent<DraftPage> cut) =>
