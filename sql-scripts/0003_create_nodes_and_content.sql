...
CREATE TABLE dbo.KnowHowToAI_NodeContent (
    ...
    CONSTRAINT PK_KnowHowToAI_NodeContent
        PRIMARY KEY CLUSTERED (SnapshotId, NodeId, AudienceId),
    ...
    CONSTRAINT FK_KnowHowToAI_NodeContent_Node FOREIGN KEY (SnapshotId, NodeId)
        REFERENCES dbo.KnowHowToAI_Node (SnapshotId, NodeId),
    CONSTRAINT FK_KnowHowToAI_NodeContent_Audience FOREIGN KEY (SnapshotId, AudienceId)
...
CREATE NONCLUSTERED INDEX IX_KnowHowToAI_NodeContent_Revision
    ...
    INCLUDE (NodeId, AudienceId, ContentMode, IsDeleted);

CREATE NONCLUSTERED INDEX IX_KnowHowToAI_NodeContent_Audience
    ON dbo.KnowHowToAI_NodeContent (SnapshotId, AudienceId, IsDeleted)
...
CREATE TABLE dbo.KnowHowToAI_ContentDependency (
    ...
    TargetAudienceId                NVARCHAR(50)     COLLATE Latin1_General_100_BIN2 NOT NULL,
    SourceNodeId                UNIQUEIDENTIFIER NOT NULL,
    SourceAudienceId                NVARCHAR(50)     COLLATE Latin1_General_100_BIN2 NOT NULL,
    ...
    CONSTRAINT CK_KnowHowToAI_ContentDependency_NotSelf CHECK (
        TargetNodeId <> SourceNodeId OR TargetAudienceId <> SourceAudienceId
    ),
    CONSTRAINT FK_KnowHowToAI_ContentDependency_Target
        FOREIGN KEY (SnapshotId, TargetNodeId, TargetAudienceId)
        REFERENCES dbo.KnowHowToAI_NodeContent (SnapshotId, NodeId, AudienceId),
    CONSTRAINT FK_KnowHowToAI_ContentDependency_Source
        FOREIGN KEY (SnapshotId, SourceNodeId, SourceAudienceId)
        REFERENCES dbo.KnowHowToAI_NodeContent (SnapshotId, NodeId, AudienceId)
);

CREATE NONCLUSTERED INDEX IX_KnowHowToAI_ContentDependency_Source
    ON dbo.KnowHowToAI_ContentDependency (SnapshotId, SourceNodeId, SourceAudienceId)
    INCLUDE (SourceContentRevisionId, TargetNodeId, TargetAudienceId);
